...
def predict(model,location):
    lat = location['lat']
    lng = location['lng']
    print("Predicting weather for location " + str(lat) + " " + str(lng))
    prediction_data = functionsWrappup.getLastWeatherReportForLocation({'lat':lat, 'lng':lng})
    print("Prediction data collected, total nr of records " + str(len(prediction_data)))
    prepared_data = prepare_data(prediction_data)
    print("Data prepared for prediction")
    prediction = model.predict(prepared_data)
    print("Weather predicted, total number of predictions made " + str(len(prediction)))
    processed_prediction = []
    for el in prediction:
        row = []
        for a in el:
            row.append(round(abs(a),2))
        processed_prediction.append(row)
